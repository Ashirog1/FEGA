...
def main():
    loc = os.getcwd()
    print("loc", loc)
    files = glob.glob(loc + r"/script/txtData/*")
    for f in files:
        print(f)
        os.remove(f)
    print("reading path", loc + r"/script/Data_k-1_2/" + "para.csv")
    try:
        paramater = pd.read_csv(loc + r"/script/Data_k-1_2/" + "para.csv")
...
#        exit(0)
    ...
    for ind, row in paramater.iterrows():
        ...
        for filename in os.listdir(loc + r"/script/Data_k-1_2/"):
            # if filename.endswith(".xlsx"):
            #     continue
            if str(readfile) == filename[: len(filename) - 4]:
                df = pd.read_csv(loc + r"/script/Data_k-1_2/" + filename)
                ...
                try:
                    with open(
                        loc + r"/script/txtData/" + filename[: len(filename) - 3] + "txt", "w"
                    ) as outfile:
                        try:
                            print(
                                (number_truck),
